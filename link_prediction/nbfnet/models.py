...
class NBFNet(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dims, num_relation, message_func="distmult", aggregate_func="pna",
                 ...
                 dependent=True, remove_one_hop=False, num_beam=10, path_topk=10, edge_embed_dim=None):
        ...
        for i in range(len(self.dims) - 1):
            self.layers.append(layers.GeneralizedRelationalConv(self.dims[i], self.dims[i + 1], num_relation,
                                                                ...
                                                                activation, dependent, edge_embed_dim=edge_embed_dim))
    ...
    def bellmanford(self, data, h_index, r_index, separate_grad=False):
        ...
        if data.x is not None:
            boundary_add = data.x.unsqueeze(0).expand(batch_size, -1, -1) # data.x needs shape (num_nodes, self.dims[0])
            boundary = boundary + boundary_add
            boundary = boundary.to(h_index.device)
...
class EdgeGraphsNBFNet(NBFNet):
    ...
    def __init__(
        ...
        self.edgegraph_model = MPNN(
            input_dim=1, hidden_dim=edge_embed_dim, num_layers=edge_embed_num_layers, edge_model=edge_model, edge_dim=edge_dim
        )
...
class StructuralFeatureNBFNet(NBFNet):
    def __init__(
        self,
        input_dim,
        hidden_dims,
        ...
        self.up_node_emb = torch.nn.Embedding(
                    1, input_dim
                ) # same embedding for all user product nodes


    def forward(self, data, batch):
        
        data.x = self.up_node_emb.weight.repeat((data.num_nodes, 1)) # (num_nodes, input_dim)
        data.x = None

        return super().forward(data, batch)

class PriceFeatureNBFNet(NBFNet):
    def __init__(
        self,
        input_dim,
        hidden_dims,
        ...
        self.price_normalizer = nn.BatchNorm1d(1)

    def forward(self, data, batch):
        x = data.x_price.unsqueeze(-1)

        # Replace Inf and NaN Values
        x_clean = torch.nan_to_num(x, nan=0.0, posinf=1e10, neginf=-1e10)

        x_normalized = self.price_normalizer(x_clean)
        x_projected = self.phi(x_normalized)
        data.x = x_projected

        return super().forward(data, batch)

class NormalizedFeatureNBFNet(NBFNet):
    def __init__(
        self,
        input_dim,
        hidden_dims,
    ...
    def forward(self, data, batch):
        x = self.phi(data.x_normalized.unsqueeze(-1)) # (num_feat, num_nodes) -> (num_feat, num_nodes, input_dim)
        x = x.mean(dim=0) # (num_feat, num_nodes, input_dim) -> (num_nodes, input_dim)
        data.x = self.rho(x) # (num_nodes, input_dim)

        return super().forward(data, batch)

class RawFeatureNBFNet(NBFNet):
    def __init__(
        self,
        input_dim,
        hidden_dims,
        ...
        self.raw_linear = torch.nn.Linear(1, input_dim)

    def forward(self, data, batch):
        xs = []
        for i in range(data.x_raw.size(0)):
            x = data.x_raw[i].unsqueeze(-1)
            x = self.raw_linear(x)
            xs.append(x)
...
class GaussianFeatureNBFNet(NBFNet):
    def __init__(
        self,
        input_dim,
        hidden_dims,
    ...
    def forward(self, data, batch):
        data.x = data.x_gaussian

        return super().forward(data, batch)

class LLMFeatureNBFNet(NBFNet):
    def __init__(
        self,
        input_dim,
        hidden_dims,
        ...
        self.linear = torch.nn.Linear(384, input_dim)

    def forward(self, data, batch):
        data.x = self.linear(data.x_emb) # (num_nodes, input_dim)
